# THE MASSIVE DATA BOOK: STRUCTURAL VARIANT EVALUATOR (TP53-SVE)
**DO NOT SUMMARIZE. This document contains the unabridged, raw, complete dataset of every calculation performed in this study.**

## PART 1: THE FOUNDATIONAL PARAMETERS

### 1.1 Structural Domain Boundaries (Amino Acid Index Limits)
The p53 protein (UniProt P04637) is defined by the following strict domain boundaries used in all coordinate subset calculations:
...
### 1.2 Target Mutation Definitions
The complete matrix of the 50 investigated variants:
...
### 1.3 The AlphaFold Predictive Engine Algorithm
We replaced the sequence character at `index = Position - 1` with the `Mut_Residue` character and encoded this as a JSON array. AlphaFold 3 computationally folded these using deep learning multiple-sequence-alignment evolutionary heuristics, producing spatial `x, y, z` scalar coordinates for every atom in the backbone (N, Calpha, C, O) and sidechains. The confidence metric (pLDDT) was assigned per atom.

## PART 2: THE FAILURE OF GENERIC GEOMETRY (PHASE 1 & 2)

### 2.1 The Kabsch Superposition Mathematical Function
To align coordinates, we extracted strictly the `Calpha` atoms. The Kabsch algorithm generates a cross-covariance matrix `H = sum(P_i * Q_i^T)`. Singular Value Decomposition `H = U * S * V^T` yields the optimal rotation matrix `R = V * U^T`. The Root Mean Square Deviation (RMSD) is calculated from the transformed coordinates:
`RMSD = sqrt( (1/393) * sum[ (X_wt - X_mut)^2 + (Y_wt - Y_mut)^2 + (Z_wt - Z_mut)^2 ] )`

### 2.2 Global RMSD Raw Output Data
Note the mathematical paradox: Benign polymorphism P72R generates a 37.08 Å deviation, ranking higher than deadly clinical hotspots.
...
### 2.3 Domain-Isolated Coordinate Mapping
We filtered the `Calpha` Cartesian coordinates strictly to the domain boundaries (e.g., exclusively indices 102 to 292 for the DBD) and calculated isolated RMSD values. This proved mutations like L344R destroy the TET domain but leave the DBD completely unaffected.

### Raw Domain RMSD Matrix (Ångstroms):
...
## PART 3: RAW PER-RESIDUE DISPLACEMENT MATRICES

The following datasets contain the exact Euclidean displacement (in Ångstroms) of every single amino acid in the protein for specific key mutations. This proves exactly which structural loop exploded.

## PART 4: SECONDARY STRUCTURE & SOLVENT PHYSICS (PHASE 3 TIER 1)

### 4.1 Backbone Angle Deformations (Secondary Structure)
We calculated geometric distances `d(i, i+3)`. If `d < 6.0 Å`, we assigned an Alpha-Helix. If `d(i, i+2) > 6.0 Å` and `d(i, i+3) > 8.0 Å`, we assigned a Beta-Sheet. Otherwise, Random Coil. The raw data counts how many of these structural elements dissolved.

### Raw Secondary Structure Melting Data:
...
### 4.2 Hydrophobic Solvent Accessible Surface Area (SASA)
Using the Shrake-Rupley algorithm, we simulated rolling a 1.4 Å water sphere over the atomic map. We exclusively tracked the surface area expansion of purely hydrophobic residues (Val, Ile, Leu, Phe, Met). When a pocket expands, it breaks the core packing and exposes these to the solvent.

### Raw Solvation Expansion Data:
...
### 4.3 Local vs Global Impact Ratios
We defined the "Local Sphere" as exactly ±10 residues indexing outward from the mutation site. The ratio divides Local RMSD by Global RMSD. Values > 2.0 mean the damage is strictly quarantined to the mutation epicenter.

### Raw Blast Radius Ratio Data:
Mutation,Position,Overall_RMSD,Local_Mean_Disp,Global_Mean_Disp,Local_Max_Disp,Global_Max_Disp,Local_Global_Ratio,Impact_Class,Local_Residues_Above_5A,Global_Residues_Above_5A

W23R,23,36.4377,72.5497,28.0135,83.3599,91.897,2.5898,Locally Destructive,21,371

L344R,344,22.5967,41.442,16.6925,51.826,54.6317,2.4827,Locally Destructive,21,328

L22F,22,29.3793,54.4775,24.5138,61.765,65.4145,2.2223,Locally Destructive,21,372

P47S,47,22.0739,28.1805,14.9761,49.5407,55.8668,1.8817,Uniform Impact,21,261

P72R,72,37.0815,47.8787,29.2601,59.3903,103.8021,1.6363,Uniform Impact,21,372

R342P,342,36.0602,43.1554,30.0324,52.2195,74.4166,1.437,Uniform Impact,21,372

N345S,345,33.1697,32.4188,29.5675,39.7479,70.2155,1.0964,Uniform Impact,21,372

R337H,337,32.9437,28.8064,28.0164,40.0017,79.3614,1.0282,Uniform Impact,21,372

E285K,285,32.9137,26.2965,26.5084,39.5282,106.235,0.992,Uniform Impact,21,372

K382R,382,31.7023,25.1559,27.0341,38.18,74.396,0.9305,Uniform Impact,21,368

R282Q,282,31.5187,25.1391,27.2155,30.6464,72.7919,0.9237,Uniform Impact,21,372

V157F,157,32.2828,24.8085,29.0214,32.5661,67.5769,0.8548,Uniform Impact,21,372

D281G,281,32.7702,23.3685,27.7827,27.6894,79.163,0.8411,Uniform Impact,21,371

R213Q,213,8.2168,5.3111,6.3803,7.0265,29.1086,0.8324,Uniform Impact,14,166

R158L,158,32.2115,23.3339,28.4132,33.7985,74.4303,0.8212,Uniform Impact,21,371

P278S,278,36.5299,24.4088,31.2627,28.7916,88.0917,0.7808,Uniform Impact,21,372

V272M,272,28.2678,18.2565,23.985,22.6822,86.01,0.7612,Uniform Impact,21,355

Y220C,220,22.1159,11.5955,17.0181,17.1962,80.2296,0.6814,Uniform Impact,21,330

H179R,179,31.2106,16.8501,25.1832,18.2811,85.8348,0.6691,Uniform Impact,21,370

R175H,175,31.8538,16.5994,27.2095,18.4581,81.1783,0.6101,Uniform Impact,21,368

R273L,273,30.1157,14.935,25.0053,26.7439,98.8411,0.5973,Uniform Impact,19,358

R158H,158,33.4055,17.0549,28.9769,20.0273,81.3777,0.5886,Uniform Impact,21,370

R273C,273,29.3747,14.1501,24.3176,26.168,111.6015,0.5819,Uniform Impact,19,361

H193R,193,22.7217,10.1812,17.9479,14.93,67.7954,0.5673,Uniform Impact,20,330

V143A,143,35.4997,16.5489,29.4972,20.8018,119.164,0.561,Uniform Impact,21,372

R249S,249,34.2997,14.996,26.8783,21.9361,114.0848,0.5579,Uniform Impact,21,365

A138V,138,14.2027,6.6789,11.9835,8.3563,33.7593,0.5573,Uniform Impact,21,311

R175G,175,32.1425,13.6795,25.1917,24.469,99.737,0.543,Uniform Impact,21,362

C135Y,135,20.5619,7.8682,14.7141,10.2606,77.9822,0.5347,Uniform Impact,21,289

G245D,245,32.198,15.0158,29.166,25.1258,72.6702,0.5148,Uniform Impact,21,371

R280K,280,24.7245,9.9045,19.4858,16.9278,74.1967,0.5083,Uniform Impact,16,335

L194R,194,32.8773,14.3856,28.8471,19.6905,74.2709,0.4987,Globally Destabilizing,21,371

G245S,245,33.4872,13.8141,28.3947,16.2585,76.5241,0.4865,Globally Destabilizing,21,372

T125M,125,26.5572,9.2305,19.5752,12.6818,98.683,0.4715,Globally Destabilizing,21,337

M237I,237,19.7161,7.6671,16.483,10.5129,48.3112,0.4652,Globally Destabilizing,21,325

Y220S,220,32.0651,12.2089,26.5165,21.4896,79.2522,0.4604,Globally Destabilizing,20,362

A189V,189,18.9269,6.8097,14.876,11.4524,54.9211,0.4578,Globally Destabilizing,16,316

N239D,239,22.0229,8.2556,18.1892,11.5091,65.9711,0.4539,Globally Destabilizing,20,332

R248Q,248,22.192,8.2225,19.1926,12.3804,56.1675,0.4284,Globally Destabilizing,18,353

R175C,175,25.8032,8.4754,21.751,18.3389,71.5208,0.3897,Globally Destabilizing,13,350

S241F,241,37.8149,12.5103,32.4336,22.7166,118.9661,0.3857,Globally Destabilizing,20,369

C176F,176,27.554,7.5535,20.2001,9.983,114.2793,0.3739,Globally Destabilizing,20,332

R248W,248,32.6174,9.9246,26.5665,13.7748,98.4201,0.3736,Globally Destabilizing,21,356

R282W,282,28.8514,9.3552,25.5701,12.7443,106.1541,0.3659,Globally Destabilizing,21,367

R249G,249,35.1811,10.7596,30.3638,23.1341,93.9354,0.3544,Globally Destabilizing,19,363

K132R,132,17.7668,4.4462,13.1168,5.4897,46.7445,0.339,Globally Destabilizing,5,209

R273H,273,20.9865,6.0312,18.0685,11.2668,52.9083,0.3338,Globally Destabilizing,12,345

I195T,195,32.5522,9.0339,27.6795,15.7558,88.3443,0.3264,Globally Destabilizing,18,365

N247D,247,30.7411,7.1727,23.2418,11.9104,123.4064,0.3086,Globally Destabilizing,16,353

R248L,248,21.5996,5.1103,18.9879,11.3179,78.5527,0.2691,Globally Destabilizing,11,364



### 4.4 Distance Matrix: 8-Ångstrom Contact Network
We generated an adjacency list mapping every `Calpha` atom located within an 8.0 Å Euclidean sphere of every other atom. The disruption matrix tracks precisely how many of these rigid ties snapped during unfolding.

### Raw Contact Disruption Matrix:
...
## PART 5: THE NOVEL TARGET-SPECIFIC ALGORITHMS (PHASE 3 TIER 2)

### 5.1 Fold Conservation: The TM-Score
The Zhang & Skolnick (2004) Template Modeling score neutralizes massive swinging loops like P72R by modifying the distance denominator. 
`d0 = 1.24 * cube_root(L - 15) - 1.8`
`TM = (1/L) * sum(1 / (1 + (d_i / d0)^2))`

### Raw TM-Score Normalization Data:
...
### 5.2 p53-DBCA: DNA-Binding Competence Assessment
We hardcoded the exact p53 biology into 5 distinct mathematical probes to identify the *mechanism* of cancer rather than just the geometry.
Probe 1 (Zinc Tetrahedron): `score = 25 * exp(-mean_disp_zinc/3) * exp(-geom_dev_zinc/5)` extracted from indices 176, 179, 238, 242.
Probe 2 (DNA Hooks): `score = 25 * exp(-weighted_mean_disp_dna/4)` extracted from indices 248, 273, 120, 241, 280, 281, 283.
Probe 3 (L2/L3 Loops): Extracted local RMSD of indices 163-195 and 237-250.
Probe 4 (H-Bonds): Tracked Oxygen-Nitrogen backbone distances < 3.5 Å.
Probe 5 (Core Density): Volumetric packing.

### Raw p53-DBCA Probe Scores:
...
## PART 6: THERMODYNAMICS & MACHINE LEARNING CLASSIFICATION

### 6.1 TP53-ARES: Atomistic Residue Energy Scoring (Miyazawa-Jernigan)
Instead of tracking distance, we tracked probability energy. For every contact ruptured (from Part 4.4), we mapped the amino acid pair (e.g., Arginine-Alanine) to the 20x20 statistical potential matrix published by Miyazawa & Jernigan (1996) `e_ij`. The sum of all lost contacts equals the specific thermodynamic structural destabilization (Delta-Delta E).
We combined this with Breadth-First-Search wavefront propagation decay equations through the adjacency list.

### Raw ARES Thermodynamic Decay Matrix:
...
### 6.2 TP53-SVE: Structural Variant Evaluator (Fisher's LDA)
To definitively answer 'which mutation is pathogenic,' we fed a 50x34 high-dimensional matrix into a standard linear machine learning model.
The 34 features included ALL the data columns from all the CSVs listed above, plus the BLOSUM62 sequence substitution score.
We used Fisher's Linear Discriminant Analysis. We calculated the pooled within-class covariance matrix `S_w` and the difference in mean vectors between benign and pathogenic controls `(m_p - m_b)`. The optimal projection vector (hyperplane) was calculated linearly: `W = inv(S_w) * (m_p - m_b)`.

After projecting all 50 mutations onto this vector, the training AUC equaled exactly 1.000. It separated the deadly hotspot mutations perfectly from the benign P47S/P72R polymorphisms.

### 6.3 Final SVE Classification Vector Data for All 50 Mutations:
This is the ultimate output of the entire project timeline.
